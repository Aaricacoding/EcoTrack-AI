...
class TestAuthEndpoints:
    ...
    def test_login_valid_credentials(self, registered_token):
...
class TestCarbonEndpoints:
    ...
    def test_anonymous_calculation_succeeds(self, carbon_payload):
    ...
    def test_anonymous_total_equals_sum_of_parts(self, carbon_payload):
    ...
    def test_negative_car_km_rejected(self, carbon_payload):
        """Negative car_km_per_week must return 422 Unprocessable Entity."""
        carbon_payload["transport"]["car_km_per_week"] = -100
        res = client.post("/api/carbon/calculate/anonymous", json=carbon_payload)
        assert res.status_code == 422

    def test_invalid_fuel_type_rejected(self, carbon_payload):
        """Invalid fuel type must return 422."""
        carbon_payload["transport"]["car_fuel_type"] = "nuclear"
        res = client.post("/api/carbon/calculate/anonymous", json=carbon_payload)
        assert res.status_code == 422

    def test_authenticated_calculation_saves_to_db(self, registered_token, carbon_payload):
    ...
    def test_history_returns_saved_records(self, registered_token, carbon_payload):
...
class TestPredictionsEndpoint:
    ...
    def test_forecast_returns_six_months(self):
...
class TestTipsEndpoint:
    ...
    def test_tips_returns_list(self):
